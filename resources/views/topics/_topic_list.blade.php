      ...
      <li class="media">
        <div class="media-left">
          <a href="{{ route('users.show', [$topic->user_id]) }}">
            <img class="media-object img-thumbnail mr-3" style="width:53px;height:52px;" src="{{ $topic->user->avatar }}" title="{{ $topic->user->name }}">
          </a>
        </div>
        ...
        <div class="media-body">
          ...
          <div class="media-heading mt-0 mb-1">
            ...
            <a href="{{ route('topics.show', [$topic->id]) }}" class="float-right">
              <span class="badge badge-secondary"> {{ $topic->reply_count }}</span>
            </a>
          </div>
          ...
          <small class="media-body meta text-secondary">
            ...
            <span class="timeago" title="最后活跃于：{{ $topic->update_at }}">{{ $topic->updated_at->diffForHumans() }}</span>
          </small>
